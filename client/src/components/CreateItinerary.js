  ...
  useEffect(() => {
    const getAllSites = async (borough) => {
      const { data } = await axios.get(
        "http://localhost:3001/siteBorough/" + borough
      );
      let sites = [];
      sites = data.map((site) => {
        let siteInfo = {
    ...
    return (
      <div>
        ...
        <div className="create-itinerary-paragraph">
          <p>
            Select the sites which you would like to visit on your trip to NYC!
            Then click the Load Itinerary button to see your itinerary generated
            in the order in which you selected your sites.
          </p>
        </div>
        {user ? (
          <div>
            <form>
              {manhattan.length !== 0 && <h2>Manhattan:</h2>}
              {checkManhattan}
              <br />
              {staten.length !== 0 && <h2>Staten Island:</h2>}
              {checkStaten}
              <br />
              {brooklyn.length !== 0 && <h2>Brooklyn:</h2>}
              {checkBrooklyn}
              <br />
              {bronx.length !== 0 && <h2>Bronx:</h2>}
              {checkBronx}
              <br />
              {queens.length !== 0 && <h2>Queens:</h2>}
              {checkQueens}
              <br />
              <button
                onClick={(e) => {
              ...
              {itinerary.length > 1 && itinerary.length < 13 ? (
                        ...
                        !loadingItinerary && (
                          <button
                            onClick={async (e) => {
                              ...
                              try {
                                await axios.get(
                                  "http://localhost:3001/addItinerary/" +
                                    user.uid,
                                  {
                                    params: {
                                      itinerary: JSON.stringify(itinerary),
                                ...
                                alert(
                                  "Error: You already saved this itinerary"
                                );
                              }
                            }}
                          >
                        ...
                        !loadingItinerary && (
                          <button
                            onClick={async (e) => {
                              ...
                              try {
                                await axios.get(
                                  "http://localhost:3001/deleteItinerary/" +
                                    user.uid,
                                  {
                                    params: {
                                ...
                                alert(
                                  "Error: You have not saved this itinerary"
                                );
                              }
                            }}
                          >
                ...
                <div>
                  {!loadingItinerary && (
                    <p>
                      {itinerary.length < 2
                        ? "Add at least two stops to your itinerary!"
                        : "Only up to 12 stops can be added to your itinerary"}
                    </p>
                  )}
                </div>
              )}
          ...
          <p>
            <Link to="/signin">Login</Link> to create your own itinerary
          </p>
        )}
      </div>
    );
